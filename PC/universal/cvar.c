...
/* Original path: /Volumes/BigCheese/ Source/AspyrP4/CoD/Source/universal/cvar.c */
/* No exact function-to-file mapping was present for this file in the decoded xSYM tables. */
// cvar.c -- dynamic variable tracking

#include "../game_mp/q_shared.h"
...
extern fileHandle_t FS_FOpenTextFileWrite(const char *filename);
...
void Cmd_Exec_f(void) {
	...
	if (Cmd_Argc() != 2) {
		Com_Printf("usage: exec <filename.cfg>\n");
		return;
	}

	Q_strncpyz(filename, Cmd_Argv(1), sizeof(filename));

	// Force .cfg extension if not present
	int flen = strlen(filename);
	if (flen < 4 || Q_stricmp(filename + flen - 4, ".cfg") != 0) {
		I_strncat(filename, sizeof(filename), ".cfg");
	}
...
void Com_WriteConfig_f(void) {
	...
	f = FS_FOpenTextFileWrite(filename);
	if (!f) {
